...
public class SugarRushMode extends JFrame {
	/**
	 * 
	 */
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SugarRushMode frame = new SugarRushMode();
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent ev) {
				...
				if (keycode == KeyEvent.VK_W) {
					cursor.setLocation(cursor.getX(), cursor.getY() - (int)(Math.random() * 51));
				}
				//Move Left
				if (keycode == KeyEvent.VK_A) {
					cursor.setLocation(cursor.getX() - (int)(Math.random() * 51), cursor.getY());
				}
				//Move Down
